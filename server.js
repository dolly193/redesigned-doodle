...
const { client, GUILD_ID, DISCORD_CLIENT_ID, DISCORD_CLIENT_SECRET, SITE_URL, sendProofForVerification, processVerificationAction } = require('./index.js');
...
app.use(express.urlencoded({ extended: true })); // Adiciona este middleware para analisar corpos de requisição URL-encoded
...
// Anexa o client do Discord e o io ao app para que possam ser acessados em outros módulos
...
app.get('/order/create/:productId', async (req, res) => {
    ...
    try {
        ...
        res.render('awaiting-payment', { product: productData, order: newOrder, pix_key: process.env.PIX_KEY });
...
app.get('/order/awaiting-payment/:orderId', async (req, res) => {
    ...
    res.render('awaiting-payment', { product, order, pix_key: process.env.PIX_KEY });
});

// --- ROTAS DE VERIFICAÇÃO (RESTAURADAS) ---

// Rota para exibir a página de verificação
...
// Rota para processar a ação (Aprovar/Recusar/Entregar)
app.post('/verify/action/:verificationId', async (req, res) => {
    ...
    verificationTokens.delete(verificationId); // Remove o token após o uso
...
async function startServer() {
    await createTables(); // 1. Garante que as tabelas do DB existam
    await client.login(process.env.DISCORD_TOKEN); // 2. Loga o bot
    // 3. Inicia o servidor web APÓS o bot estar logado
    server.listen(port, () => {
...
module.exports = { app, io, verificationTokens };
