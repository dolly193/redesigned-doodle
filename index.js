...
async function updateFixedShoppingPanel() {
    ...
    // CORREÇÃO: Adiciona uma verificação para garantir que panelConfig não é nulo.
    if (panelConfig && panelConfig.panelMessageId && panelConfig.panelChannelId) {
        try {
 ...
 */
async function processPaymentConfirmation(guild, channelId, productId, adminUser, contextType = 'ticket') {
    ...
    // CORREÇÃO: Apenas cria o canal de entrega se a compra foi via ticket do Discord.
    // Para compras via site, o canal de chat já é criado em 'processVerificationAction'.
    if (contextType === 'ticket') {
        ...
        // Envia a notificação de entrega para o admin
...
async function processVerificationAction(action, context, adminUser) {
    ...
    if (action === 'approve') {
        if (type === 'ticket') {
            return await processPaymentConfirmation(guild, channelId, productId, await client.users.fetch(OWNER_ID));
        } else if (type === 'site') {
            ...
            // Atualiza o pedido com o status 'approved' e o ID do canal criado
            await updateOrderStatus(orderId, 'approved', { ticketChannelId: ticketChannel.id });

            // Envia a notificação para o admin marcar como entregue
            await sendDeliveryNotification({
                context: { type: 'site', orderId: orderId, userId: userId, productName: targetOrder.productName }
            });

            // Notifica o frontend sobre a aprovação via Socket.IO
            const { app } = require('./server.js');
            const io = app.get('socket_io');
            io.to(orderId).emit('order_status_update', { status: 'approved' });
        ...
        if (type === 'ticket') {
            ...
            // Notifica o frontend sobre a recusa via Socket.IO
            const { app } = require('./server.js');
            const io = app.get('socket_io');
            io.to(orderId).emit('order_status_update', { status: 'declined' });
...
client.on(Events.InteractionCreate, async (interaction) => {
    if (interaction.isChatInputCommand()) { // Lida com comandos de barra
        ...
        if (action === 'confirm-delete') {
            ...
            await processPaymentConfirmation(interaction.guild, interaction.channel.id, id, interaction.user, 'ticket');
...
async function sendProofForVerification(verificationData) {
    try {
        ...
        verificationTokens.set(verificationId, verificationData);
...
async function sendDeliveryNotification(deliveryData) {
    try {
        ...
        const tokenPayload = {
            ...
            details: { productName: deliveryData.context.productName }
        };
    ...
    if (message.channel.topic && message.channel.topic.startsWith('Chat do Pedido do Site')) {
        ...
        if (orderIdMatch) {
            ...
            await addMessageToOrder(orderId, newMessage);
                ...
                // Emite a mensagem para o chat do site via Socket.IO - CORREÇÃO
                const { app } = require('./server.js');
                const io = app.get('socket_io');
